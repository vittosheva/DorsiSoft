...
@section('title', __('Invoice') . ' ' . $document->code)
@section('doc-type', __('Invoice'))
        ...
        <tr>
            ...
            <td class="billing-right">
                <table class="detail-table">
                    ...
                    {{-- <tr>
                        <td class="detail-key">{{ __('Due date') }}</td>
                        <td class="detail-val">{{ $pdfDateFormatter::formatDate($document->due_date) }}</td>
                    </tr> --}}
                    <tr>
                        <td class="detail-key">{{ __('Currency') }}</td>
                        <td class="detail-val">{{ $document->currency_code ?? 'USD' }}</td>
                    </tr>
                    @if($document->salesOrder?->code)
                        <tr>
                            <td class="detail-key">{{ __('Sales order') }}</td>
                            <td class="detail-val">{{ $document->salesOrder->code }}</td>
                        </tr>
                    @endif
            ...
            <td class="totals-cell">
                <table class="totals-table">
                    ...
                    @if($document->paid_amount > 0)
                        <tr>
                            <td class="totals-paid-label">{{ __('Paid') }}</td>
                            <td class="totals-paid-value">{{ number_format((float) $document->paid_amount, 2) }}</td>
                        </tr>
                        @if($document->total > $document->paid_amount)
                            <tr>
                                <td class="totals-due-label">{{ __('Balance due') }}</td>
                                <td class="totals-due-value">
                                    {{ number_format((float) bcsub((string) $document->total, (string) $document->paid_amount, 4), 2) }}
                                </td>
                            </tr>
                        @endif
                    @endif
