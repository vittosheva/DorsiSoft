    ...
    @if (count($pendingItems) > 0)
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900/40">
            <div class="overflow-x-auto">
                <div class="min-w-260">
                    {{-- Column headers --}}
                    <div class="grid grid-cols-12 gap-3 bg-primary-900 px-5 py-3.5 text-sm font-bold uppercase tracking-[0.14em]- text-gray-100 dark:bg-gray-950 fi-document-table-header">
                            ...
                            <div class="grid grid-cols-12 items-center gap-3 px-5 py-3.5 transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/25">
                                ...
                                <div class="col-span-2 flex items-center gap-1.5 justify-center">
                                    <input
                                        ...
                                        name="discount_value_{{ $item['discount_value'] ?? '0' }}"
                                        min="0"
                                        step="0.01"
                                        wire:change="updateItemField('{{ $item['_key'] }}', 'discount_value', $event.target.value)"
                                        @disabled($isReadOnly)
                                        class="w-full rounded-lg border border-gray-300 bg-white px-1.5 py-2 text-center text-sm font-medium text-gray-800 transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 dark:border-gray-600 dark:bg-gray-700/70 dark:text-white"
                                    />
                                    <select
                                        name="discount_type_{{ $item['discount_type'] ?? 'percentage' }}"
