...
final class CreditNoteItems extends Component
{
    ...
    public function mount(?int $creditNoteId = null, ?string $operation = null, int $minimumItemsCount = 0, ?string $minimumItemsValidationMessage = null): void
    {
    ...
    #[On('credit-note-items:load-from-invoice')]
    ...
    public function persistItems(int $documentId): void
    {
        ...
        app(DocumentItemsPersistor::class)->sync(
            [
                'item_model' => CreditNoteItem::class,
                'tax_model' => CreditNoteItemTax::class,
                'document_fk' => 'credit_note_id',
                'item_tax_fk' => 'credit_note_item_id',
            ],
            $documentId,
            $this->pendingItems,
        );
        ...
        $creditNote = CreditNote::with(['items.taxes'])->find($documentId);
    ...
    private function recalculate(): void
    {
        ...
        $creditNote = CreditNote::with(['items.taxes'])->findOrFail($this->creditNoteId);
        app(CreditNoteTotalsCalculator::class)->recalculate($creditNote);
    }
}
